...
The try-on experience should preserve a provider-neutral server contract, clearly distinguish simulation mode from provider-backed generation in the UI, and keep presentation choice explicit so the fallback can place different hairline/volume shapes.
